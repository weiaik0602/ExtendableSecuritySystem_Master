...
void UART_Receive_Buffer(){
  if(uart_receive_position >= BUFFER_SIZE){
    uart_receive_position = 0;
  }
  uart_receive_buffer[spi_receive_position].board = uart_receive[0];
  uart_receive_buffer[spi_receive_position].module = uart_receive[1];
  uart_receive_buffer[spi_receive_position].size = uart_receive[2];
  uart_receive_buffer[spi_receive_position].data = uart_receive[3];
